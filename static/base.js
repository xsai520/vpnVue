...
/*		
	...
	base.form.autoForm = function(option){
		...
		self.createTable = function(){
			...
			if(self.layoutOption){base.setProperty(self.layoutContainer,self.layoutOption);}
			...
			switch(self.type){
				...
				case "file":
					...
					var btn = base.form.button({
						...
						attr:[{"name":"onclick","value":"return false;"}],
						clickEvent:function(){
							$(self.element).click();
							return false;
						}
	...
	base.form.button = function(option) {
		...
		self.create = function() {
			...
			if(self.icon){
				if(self.iconStyle){
					$(self.element).html("<i class='"+self.icon+"' style='"+self.iconStyle+"'></i>");
				}else{
					$(self.element).html("<i class='"+self.icon+"'></i>");
				}
	...
	base.form.dateRange = function(option){
		var self = {};
		self.dateOption = option.dateOption?option.dateOption:{format: 'YYYY-MM-DD'};
	...
	base.tip = function(option){
		...
		self.width = option.width?option.width:200;
		self.height = option.width?option.width:30;
		self.create = function(){
			var modal = base.modal({
				...
				labelStyle:"font-size:14px",
				buttons:[
					{"label":"关闭","cls":"btn btn-info","clickEvent":function(){
						modal.hide();
					}}
				]
			});
	...
	base.dragPickBox = function(option){
		...
		self.pick = function(){
			if(!self.activeBox){return;}
			var uid = $(self.dragElement).attr("uid");
			if(self.checkRepeat(uid)){base.alert("不能重复添加");return;}
   ...
   base.getDateByDay = function(dayNumber,hasTime){
	...
	base.calendar = function(option){
		...
		if(self.data){
			self.calendarOption.events = self.data;
		}
		...
		self.calendarOption.weekNumbers = true;
